...
export default function PatientProfilePage() {
  ...
  function downloadPrescriptionPdf(rx: PrescriptionRecord) {
    ...
    doc.addTwoColumn(
      "MedFlow Clinic",
      "PRESCRIPTION",
      { fontSize: 16, bold: true, color: [37, 99, 235] }
    );
    doc.addText(rx.id, { fontSize: 9, color: [100, 116, 139], align: "right" });
    doc.addDivider();

    // ── Patient / Prescribed-by block ──────────────────────────────
    doc.addTwoColumn("PATIENT", "PRESCRIBED BY", { fontSize: 9, color: [100, 116, 139] });
    doc.addTwoColumn(
      patient?.full_name || "Patient",
      rx.prescriber,
      { fontSize: 10, bold: true, color: [30, 41, 59] }
    );
    doc.addTwoColumn(
      `DOB: ${patient?.dob ? formatDateIST(patient.dob) : "—"}`,
      `Date: ${rx.date}`,
      { fontSize: 9, color: [100, 116, 139] }
    );
    doc.addDivider();

    // ── Diagnosis / Medication ─────────────────────────────────────
    doc.addText("DIAGNOSIS / MEDICATION", { fontSize: 9, bold: true, color: [100, 116, 139] });
    doc.addText(rx.medication, { fontSize: 10, color: [30, 41, 59] });
    if (rx.instruction) {
      doc.addText(`Instructions: ${rx.instruction}`, { fontSize: 9, color: [100, 116, 139] });
    }
    doc.addSpacer(10);

    // ── Footer ─────────────────────────────────────────────────────
    doc.addDivider();
    doc.addText("Generated by MedFlow Clinic", { fontSize: 8, color: [150, 150, 150] });
