  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto font-sans pb-24">
      ...
      <nav className="flex items-center gap-2 text-slate-500 text-xs font-medium">
        <Link href="/appointments" className="hover:text-[#004ac6] transition">Appointments</Link>
        <ChevronRight className="w-3.5 h-3.5" />
        <Link href="/appointments" className="hover:text-[#004ac6] transition">Cardiology Consultation</Link>
        <ChevronRight className="w-3.5 h-3.5" />
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6 flex flex-col sm:flex-row items-start justify-between gap-4 shadow-sm">
        <div className="flex items-center gap-5">
          <div className="w-14 h-14 rounded-full bg-blue-50 text-[#004ac6] flex items-center justify-center font-bold text-base shrink-0 border border-blue-100">
            MC
          </div>
          <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" value={medication} onChange={(e) => setMedication(e.target.value)}
                  className="w-full h-9 pl-9 pr-3 rounded-lg border border-slate-200 text-xs focus:border-[#004ac6] outline-none" />
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div>
                ...
                <input type="text" value={dosage} onChange={(e) => setDosage(e.target.value)}
                  className="w-full h-9 px-3 rounded-lg border border-slate-200 text-xs focus:border-[#004ac6] outline-none" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-800 mb-1">Frequency</label>
                <select value={frequency} onChange={(e) => setFrequency(e.target.value)}
                  className="w-full h-9 px-2.5 rounded-lg border border-slate-200 text-xs focus:border-[#004ac6] outline-none bg-white">
          ...
          <button className="w-full py-3 border border-dashed border-slate-300 rounded-xl flex items-center justify-center gap-1.5 text-xs font-semibold text-[#004ac6] hover:bg-blue-50 transition">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm space-y-2">
            ...
            <textarea rows={4} value={pharmacyNotes} onChange={(e) => setPharmacyNotes(e.target.value)}
              ...
              className="w-full p-2.5 rounded-lg border border-slate-200 text-xs focus:border-[#004ac6] outline-none" />
          </div>
        </div>
      </div>
      ...
      <div className="fixed bottom-0 right-0 left-60 bg-white border-t border-slate-200 p-4 flex justify-between items-center z-30 px-8 shadow-lg">
        ...
        <div className="flex gap-2.5">
          ...
          <Button size="sm" onClick={handleFinalize} className="bg-[#004ac6] hover:bg-blue-700 text-white font-semibold flex items-center gap-1.5 shadow-sm">
