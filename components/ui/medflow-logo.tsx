...
export function MedFlowLogo({
  ...
  return (
    <div className={`flex items-center gap-3 ${className}`}>
      ...
      <div
        className={`relative flex items-center justify-center shrink-0 rounded-xl bg-primary text-white shadow-sm ${iconSizes[size]}`}
      >
        <div className="absolute -top-1 w-3.5 h-1.5 border-2 border-primary rounded-t-sm" />
      ...
      <div className="flex flex-col">
        <div
          className={`flex items-center text-primary ${textSizes[size]} font-[900] tracking-[-0.03em] leading-none select-none`}
        >
          <span>Med</span>
          <svg
        ...
        {showSubtitle && (
          <span className="text-[11px] font-medium text-on-surface-variant tracking-tight mt-0.5">
            {subtitle}
          </span>
        )}
      </div>
