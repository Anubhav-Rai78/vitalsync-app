...
-- clinic_id and every RLS policy scopes on it.
...
create type user_role as enum ('admin', 'doctor', 'front_desk');
...
-- Role/clinic assignment happens at registration time via the
-- register Server Action, which updates this row right after signup.
...
-- Helper: current user's clinic + role, used by every policy below.
...
-- clinics: members can read their own clinic; only admin can update it.
...
-- profiles: read anyone in your clinic; only admin manages staff.
...
create policy "admin manages profiles" on profiles
  for all using (clinic_id = public.current_clinic_id() and public.current_role() = 'admin');
create policy "self updates own profile" on profiles
  for update using (id = auth.uid());

-- Generic clinic-isolation policy, applied per table below.
...
-- audit_logs: admin-only read, system-inserted (service role) or any
-- authenticated action via the audit() trigger function below.
create policy "admin reads audit log" on audit_logs
  for select using (clinic_id = public.current_clinic_id() and public.current_role() = 'admin');
...
-- ────────────────────────────────────────────────────────────────
-- Audit logging trigger — fires on every write to a clinical table
-- ────────────────────────────────────────────────────────────────
...
create index idx_audit_logs_clinic_time on audit_logs(clinic_id, created_at desc);
